...
/*	reg_matrix permet de construire la matrice de regression linéraire qui sera utilisée à partir de toutes les classes construites 
	jusqu'ici.
	Cette matrice contient comme premières colonne les différentes observations des différentes greecques du portefeuille, en effet on 
	considère que chaque grecque représente une nouvelle observation dans le cadre de notre régression.*/

class reg_matrix : public data_frame
{
public:
	void add_product(data_frame, string, int begin, int end); 
	...
	// effectue la regression
	reg_matrix(Portfolio, Vanilla_Products, int begin, int end); 
	// La matrice de regression est construite à partir du portefeuille et des produits vanilles, les arguments begin et end jouent le même rôle 
	// que précédement
	reg_matrix();
	~reg_matrix();
};
...
alglib::real_2d_array convert_matrix(deque<deque<double> >);  