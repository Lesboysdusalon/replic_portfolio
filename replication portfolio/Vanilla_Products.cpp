...
/*	Le constructeur Vanilla_Products permet de construire un deque<double> contenant toutes les nav et les grecques des 
...
Vanilla_Products::Vanilla_Products(string ul, deque<int> maturity, deque<int> order, deque<double> strike, deque<string> type, const data_frame &data)
{
	...
	_strike = strike;
	_type = type;
	Basic_products underlying(true, ul, data);
	Basic_products risk_free_asset(false, ul, data); 
	this -> include_financial_product(underlying,ul); // On inclut dans la base de données vanilles, ie l'objet en cours de construction, le sous jacent lui même
	this -> include_financial_product(risk_free_asset,"Risk free asset"); // De même pour l'actif sans risque
		...
		for (size_t t = 0; t < maturity.size(); t++)
		{
			for (size_t k = 0; k < strike.size(); k++)
			{
				for (size_t l = 0; l < type.size(); l++)
				{
					...
					// ex label : sp_call_maturity_31_order_0_strike_10_delta. Cette labélisation permet de retrouver les données facilement.
					c += 6;  
				}
			}
		}
